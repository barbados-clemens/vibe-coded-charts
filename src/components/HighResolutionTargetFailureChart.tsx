...
const HighResolutionTargetFailureChart: React.FC<
  HighResolutionTargetFailureChartProps
> = ({ data }) => {
  ...
  const [minRunsThreshold, setMinRunsThreshold] =
    useState<MinRunsThreshold>("none");
  ...
  const applyMinRunsFilter = useCallback(
    (
      chartData: any[],
      threshold: MinRunsThreshold,
    ): {
      filteredData: any[];
      originalIntervals: number;
      filteredIntervals: number;
    } => {
      if (threshold === "none") {
        return {
      ...
      const filteredData = chartData.filter(
        (interval) => interval.total >= minRuns,
      );
  ...
  const rawChartData = useMemo(() => {
    ...
    const intervalMap = new Map<
      string,
      { total: number; failed: number; timestamp: Date }
    >();
      ...
      run.tasks?.forEach((task) => {
        if (task.target === selectedTarget) {
          ...
          intervalStart.setMinutes(
            Math.floor(intervalStart.getMinutes() / 5) * 5,
          );
          ...
          const status =
            typeof task.status === "object"
              ? parseInt(task.status.$numberLong)
              : task.status;
          if (status !== 0) {
            interval.failed += 1;
          }
    ...
    const intervals = Array.from(intervalMap.values())
      ...
      .map((interval) => ({
        ...
        failureRate:
          interval.total > 0 ? (interval.failed / interval.total) * 100 : 0,
      }));
  ...
  const displayData = useMemo(() => {
    ...
    const leftIndex = chartData.findIndex(
      (item) => item.shortTime === zoomDomain.left,
    );
    const rightIndex = chartData.findIndex(
      (item) => item.shortTime === zoomDomain.right,
    );
  ...
  const stats = useMemo(() => {
    ...
    if (dataToAnalyze.length === 0)
      return { totalRuns: 0, totalFailures: 0, overallFailureRate: 0 };

    const totalRuns = dataToAnalyze.reduce((sum, item) => sum + item.total, 0);
    const totalFailures = dataToAnalyze.reduce(
      (sum, item) => sum + item.failed,
      0,
    );
    const overallFailureRate =
      totalRuns > 0 ? (totalFailures / totalRuns) * 100 : 0;
  ...
  const handleZoomNavigation = useCallback(
    (direction: "prev" | "next") => {
      if (!zoomDomain.left || !zoomDomain.right || displayData.length === 0)
        return;

      const currentLeftIndex = chartData.findIndex(
        (item) => item.shortTime === zoomDomain.left,
      );
      const currentRightIndex = chartData.findIndex(
        (item) => item.shortTime === zoomDomain.right,
      );
      ...
      if (direction === "prev") {
        ...
        if (newMaxIndex < chartData.length) {
          setZoomDomain({
            ...
            right:
              chartData[Math.min(newMaxIndex, chartData.length - 1)].shortTime,
          });
        }
      } else {
    ...
    <div className="bg-white rounded-lg shadow p-6">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center space-x-4">
          <h2 className="text-lg font-semibold text-gray-900">
            High Resolution Target Failure Analysis
          </h2>
          <select
          ...
          <select
            ...
            onChange={(e) =>
              setMinRunsThreshold(e.target.value as MinRunsThreshold)
            }
            className="px-3 py-1 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-orange-50 p-4 rounded-lg">
          ...
          <p className="text-2xl font-semibold text-orange-900">
            {stats.overallFailureRate.toFixed(1)}%
          </p>
        </div>
      </div>
      ...
      {chartData.length === 0 ? (
        <div className="h-64 flex items-center justify-center text-gray-500">
          {selectedTarget
            ? `No data available for target "${selectedTarget}" in ${format(currentDate, "MMMM yyyy")}`
            : "No target selected"}
        </div>
      ) : (
        <div className="h-96">
      ...
      {chartData.length > 0 && (
        <div className="mt-6 bg-gray-50 p-4 rounded-lg">
          ...
          <div className="text-xs text-gray-600 space-y-1">
            <p>
              • Data shown in 5-minute intervals for high resolution analysis
            </p>
            <p>• Total intervals with data: {chartData.length}</p>
            {!zoomDomain.left ? (
              <p>
                • Full time range: {chartData[0].time} to{" "}
                {chartData[chartData.length - 1].time}
              </p>
            ) : (
              <>
                <p>
                  • Original range: {chartData[0].time} to{" "}
                  {chartData[chartData.length - 1].time}
                </p>
                <p className="text-blue-600">
                  • Zoomed view: {displayData.length} intervals (
                  {displayData[0]?.time} -{" "}
                  {displayData[displayData.length - 1]?.time})
                </p>
              </>
            ...
            {minRunsThreshold !== "none" && (
              <p className="text-orange-600">
                • Min runs filter: {minRunsThreshold} runs per interval (
                {filteredIntervals} intervals removed from {originalIntervals}{" "}
                total)
              </p>
            )}
            {stats.overallFailureRate > 10 && (
              <p className="text-red-600 font-medium">
                ⚠️ Failure rate above 10% threshold - investigation recommended
              </p>
            )}
          </div>
        </div>
